...
var INSTRUMENT_NAME = 'BTC-28JUN19'
	...
	restClient.positions().then((result) => {	
		for (var r in result){
			...
			if (result[r][a].direction == 'sell'){
				console.log('buybuy')
				restClient.cancelall().then((result) => {
		restClient.buy(INSTRUMENT_NAME,  -1 *Math.floor(s/3), ha).then((result) => {
			console.log(result);
					});
			console.log(result);
					});
				...
				restClient.cancelall().then((result) => {
		restClient.sell(INSTRUMENT_NAME, Math.floor(s/3), lb).then((result) => {
			console.log(result);
					});	     	
	});
			}
		...
		if (result[r][a].size > ((tar * tar2 )						) || result[r][a].size < (-1 * (tar * tar2) )){
			...
			if (result[r][a].direction == 'sell'){
				console.log('buybuy')
				restClient.cancelall().then((result) => {
		restClient.buy(INSTRUMENT_NAME,  -1 *Math.floor(s/2), lb).then((result) => {
			console.log(result);
					});
			console.log(result);
					});
				...
				console.log('sellsell')
				restClient.cancelall().then((result) => {
		restClient.sell(INSTRUMENT_NAME, Math.floor(s/2), ha).then((result) => {
			console.log(result);
					});
	});
			}
...
setInterval(function(){
	restClient.getopenorders(INSTRUMENT_NAME).then((result) => {
	var go = true;
	for (var a in result){	
...
if (go){
	tar = tar + tarincrease;
		restClient.sell(INSTRUMENT_NAME, tar, ha).then((result) => {
					});
		restClient.buy(INSTRUMENT_NAME, tar, lb).then((result) => {
					});
}
	});
...
var tarmult = 1;
setInterval(function(){
restClient.getorderbook(INSTRUMENT_NAME).then((result) => {
...
can = true;
tar = tarmult * tarmult * ((btcNow * ha) / 8);
setTimeout(function(){
restClient.buy(INSTRUMENT_NAME, tar, lb).then((result) => {
buying = lb;
count++;
